...
# (3,), for this case, broadcasting semantics will apply to obtain a tensor of shape (2000, 3)

print(f'xx shape: {xx.shape}')  # torch.Size([2000, 3])

# Use the nn package to define our model as a sequence of layers. nn.Sequential is a Module which contains other Modules, and applies them in sequence to produce its output. The Linear Module computes output from input using a linear function, and holds internal Tensors for its weight and bias. The Flatten layer flatens the output of the linear layer to a 1D tensor, to match the shape of `y`.
...
for t in range(2000):
    ...
    # Forward pass: compute predicted y by passing x to the model. Module objects override the __call__ operator so you can call them like functions. When doing so you pass a Tensor of input data to the Module and it produces a Tensor of output data.
...
print(f'linear_layer.weight shape: {linear_layer.weight.shape}')  # torch.Size([1, 3])
